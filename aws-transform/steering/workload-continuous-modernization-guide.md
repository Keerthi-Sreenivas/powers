...
Scheduled analyses run on **remote infrastructure only** — EventBridge Scheduler dispatching to a provisioned EC2 or Batch stack, managed entirely through `atx ct schedule` commands. Route to [workload-continuous-modernization-schedule.md](workload-continuous-modernization-schedule.md).
