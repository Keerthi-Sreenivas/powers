...
| ------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| A count shows `unavailable (read failed …)` | That sub-query (repos or findings) failed — it is **not** actually 0 | Surface the warning: credentials may have expired (`ada credentials update`) or `AWS_REGION` may not match where resources live. Re-run after fixing. |
| Connection error                            | The CLI can't reach the AWS Transform backend                        | Refresh AWS credentials (`ada credentials update`) and confirm `AWS_REGION` matches a supported region; then retry.                                    |
